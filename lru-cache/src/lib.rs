...
//! use std::mem::size_of;
//!
//! let mut cache = LruCache::new(2*size_of::<i32>());
//!
//! cache.insert(1, 10);
//! cache.insert(2, 20);
//! cache.insert(3, 30);
...
/// An LRU cache.
#[derive(Clone)]
pub struct LruCache<K: Eq + Hash, V: Sizer, S: BuildHasher = RandomState> {
    map: LinkedHashMap<K, V, S>,
    max_size: usize,
    current_size: usize,
}

impl<K: Eq + Hash, V: Sizer> LruCache<K, V> {
...
impl<K: Eq + Hash, V: Sizer, S: BuildHasher> LruCache<K, V, S> {
    ...
    /// use std::mem::size_of;
    ///
    /// let mut cache = LruCache::new(1*(size_of::<&str>()+1));
    ///
    /// cache.insert(1, "a");
    ...
    /// use std::mem::size_of;
    ///
    /// let mut cache = LruCache::new(2*(size_of::<&str>()+1));
    ///
    /// cache.insert(1, "a");
    /// cache.insert(2, "b");
    ...
    pub fn insert(&mut self, k: K, v: V) -> Option<V> {
        ...
        if let Some(ref oldv) = old_val {
            self.current_size -= oldv.size();
        }
        while self.current_size > self.capacity() && self.map.len() > 1 {
            self.remove_lru();
        }
        old_val
    }
    ...
    ///
    /// cache.set_capacity(3*(size_of::<&str>()+1));
    /// cache.insert(1, "a");
    /// cache.insert(2, "b");
    ///
    /// assert_eq!(cache.get_mut(&1), Some(&mut "a"));
    ...
    /// assert_eq!(cache.get_mut(&3), Some(&mut "c"));
    ///
    /// cache.set_capacity(1*(size_of::<&str>()+1));
    ///
    /// assert_eq!(cache.get_mut(&1), None);
    ...
    pub fn set_capacity(&mut self, capacity: usize) {
        while self.current_size > capacity && self.map.len() > 1 {
            self.remove_lru();
        }
    ...
    pub fn remove_lru(&mut self) -> Option<(K, V)> {
        let kv = self.map.pop_front();
        if let Some((_, ref v)) = kv {
            self.current_size -= v.size();
        }
        kv
    }
...
impl<K: Eq + Hash, V: Sizer, S: BuildHasher> Extend<(K, V)> for LruCache<K, V, S> {
...
impl<K: Eq + Hash, V: Sizer, S: BuildHasher> IntoIterator for LruCache<K, V, S> {
    type Item = (K, V);
...
impl<'a, K: Eq + Hash, V: Sizer, S: BuildHasher> IntoIterator for &'a LruCache<K, V, S> {
    type Item = (&'a K, &'a V);
...
impl<'a, K: Eq + Hash, V: Sizer, S: BuildHasher> IntoIterator for &'a mut LruCache<K, V, S> {
...
mod tests {
    ...
    #[test]
    fn test_put_update() {
        let mut cache = LruCache::new(1 * size_of::<i32>());
        cache.insert("1", 10);
        cache.insert("1", 19);
    ...
    fn test_change_capacity() {
        let mut cache = LruCache::new(2 * size_of::<i32>());
        assert_eq!(cache.capacity(), 2 * size_of::<i32>());
        cache.insert(1, 10);
        cache.insert(2, 20);
        cache.set_capacity(1 * size_of::<i32>());
        assert!(cache.get_mut(&1).is_none());
        assert_eq!(cache.capacity(), 1 * size_of::<i32>());
    }

    #[test]
    fn test_debug() {
        let mut cache = LruCache::new(3 * size_of::<i32>());
        cache.insert(1, 10);
        cache.insert(2, 20);
        cache.insert(3, 30);
        ...
        cache.set_capacity(2 * size_of::<i32>());
